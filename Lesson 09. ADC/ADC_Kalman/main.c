...
void Kalman_Config(void){
	SimpleKalmanFilter(1, 2, 0.01);//(Sai so do luong, Sai so gia tr uoc luong, Sai so nhieu qua trinh)
}


uint16_t lightValue;
uint16_t lightValue_Kalman;
int main() {
	RCC_Config();
	TIMER_config();
	GPIO_Config();
	...
	while(1){
		...
		lightValue_Kalman = (int)updateEstimate((int)lightValue);
